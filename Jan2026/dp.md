...
class Solution {
	func countMinSquares(_ A: inout Int) -> Int {
        //
        var dp: [Int] = Array(repeating: -1, count: A + 1)
        return minFunction(A , &dp)
	}

    func minFunction(_ n: Int,_ dp: inout [Int]) -> Int {
        if n == 0 {return 0}
        if dp[n] != -1 {
            return dp[n]
        }
        ...
        //
        while(x * x <= n) {
            answer = min(answer, 1 + minFunction(n - x * x, &dp))
            x += 1
        }
        dp[n] = answer
        return answer
